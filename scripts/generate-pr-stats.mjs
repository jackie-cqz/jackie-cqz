...
function renderSvg(pullRequests, theme) {
  const colors = theme === "dark"
    ? {
        ...
        mergedCube: { cubeTop: "#c29bff", cubeLeft: "#8250df", cubeRight: "#6e40c9" },
        openCube: { cubeTop: "#7ee787", cubeLeft: "#2ea043", cubeRight: "#238636" },
        closedCube: { cubeTop: "#c9d1d9", cubeLeft: "#8b949e", cubeRight: "#6e7681" },
      }
    : {
        ...
        mergedCube: { cubeTop: "#c7a7ff", cubeLeft: "#8957e5", cubeRight: "#6e40c9" },
        openCube: { cubeTop: "#56d364", cubeLeft: "#2da44e", cubeRight: "#1a7f37" },
        closedCube: { cubeTop: "#d0d7de", cubeLeft: "#8c959f", cubeRight: "#6e7781" },
      };
  ...
  const cubes = months.map((month, index) => {
    ...
    const stack = [
      ...Array(month.merged).fill(colors.mergedCube),
      ...Array(month.closed).fill(colors.closedCube),
      ...Array(month.open).fill(colors.openCube),
    ];
    const blocks = stack.map((cubeColors, level) => cube(x, 382, level, cubeColors)).join("");
    ...
    return `${blocks}
      ...
      ${month.count > 0 ? `<text x="${x + 10}" y="${Math.max(270, 373 - stack.length * 10)}" class="count" text-anchor="middle">${month.count}</text>` : ""}
      ${year}`;
  }).join("");
  ...
  <style>
    ...
    .count { font-size: 9px; font-weight: 700; fill: ${colors.text}; }
    .legend { font-size: 9px; font-weight: 650; fill: ${colors.muted}; }
  </style>
  ...
  <circle cx="704" cy="246" r="4" fill="${colors.open}"/>
  <text x="714" y="250" class="legend">OPEN ${open}</text>
  <circle cx="782" cy="246" r="4" fill="${colors.accent}"/>
  <text x="792" y="250" class="legend">MERGED ${merged}</text>
  <g>
